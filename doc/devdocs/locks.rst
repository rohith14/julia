...
MethodInstance updates : codegen lock
    ...
    - Set at construction, immutable:
      ...
      + sparam_vals
      + def
    - set by jl_type_infer (holding codegen lock)
      + rettype
      + inferred
      + these can be reset, see jl_set_lambda_rettype for that logic as it needs to keep functionObjectsDecls in sync
    - inInference
      + optimization to quickly avoid recurring into jl_type_infer while it is already running
      + actual state (of inferred then fptr) is protected by codegen lock
    - jlcall_api and fptr, unspecialized_ducttape
    ...
    - functionObjectsDecls
      ...
      + it is valid to use old version of this, or wait for new versions of this,
        so races are benign, as long as the code is careful not to reference other data in the lambda (such as rettype)
